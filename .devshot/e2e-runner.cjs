  ...
  try {
    ...
    for (const step of steps) {
      ...
      try {
        ...
        else if (a === 'wait') { if (sel) { await page.waitForSelector(sel, { timeout: Number(step.timeoutMs) || 8000 }); rec('wait', sel, true); } else { await new Promise((r) => setTimeout(r, Math.min(Number(step.ms) || 500, 5000))); rec('wait', (step.ms || 500) + 'ms', true); } }
